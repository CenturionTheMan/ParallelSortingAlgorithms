...
void sorting::newSortJoin(std::vector<int>& arr) {
    // std::cout << "Array size: " << (sizeof(std::vector<int>) + arr.size() * sizeof(int)) / 1024 << "kB\n";
    ...
    // const int jobs[2] = {evenComparisons, oddComparisons};
    ...
    // std::chrono::time_point<std::chrono::high_resolution_clock> now = std::chrono::high_resolution_clock::now();
{
    ...
    int place = 0;
    // std:: cout << " Jobs: " << jobs[0] << " " << jobs[1] << std::endl;
    while (!sorted) {
        ...
        // std::cout << "Wait: " << (std::chrono::high_resolution_clock::now() - now).count() << " ";
        ...
        // std::cout << "End: " << (std::chrono::high_resolution_clock::now() - now).count() << "\n";
    }
    // std::cout << std::endl;
}

void sorting::overSortJoin(std::vector<int>& arr) {
    // Create threads
    int splits = arr.size() % 10 ? std::min(16, int(arr.size() / 2)) : std::min(10, int(arr.size() / 2));
    ...
    int oddComparisons = (arrSize - 1) / 2;
{
    ...
    vecPoints range = {0, 0};
    int jobs[2] = {std::min(splits, evenComparisons), std::min(splits, oddComparisons)};
    // int jobs = std::min(splits, odd ? oddComparisons : evenComparisons);
    while (!sorted) {
        sorted = true;
        ...
        // std::cout << "Job alloc: " << (std::chrono::high_resolution_clock::now() - now).count() << " ";
        // now = std::chrono::high_resolution_clock::now();

...
void __checkRequiredOptionAndThrowErrorIfItIsNotLoaded(
    ...
    if (required_option.second)
        return;
    output::print_notification("BENCHMARK TERMINATED!");
    output::print_notification("ERROR: Configuration file misses \"" + option_key + "\"!");
...
config::configuration_t config::loadConfiguration()
{
    ...
    if (!configuration_file.good()){
        output::print_notification("BENCHMARK TERMINATED!");
        output::print_notification("ERROR: Configuration file is invalid or doesn't exist!");
